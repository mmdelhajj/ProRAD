    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {(batches || []).slice(0, 4).map(batch => (
          <div key={batch.batch_id} className="bg-white p-4 rounded-lg shadow">
            <h3 className="text-sm font-medium text-gray-500">{batch.batch_id}</h3>
            <div className="mt-2 flex justify-between">
              <span className="text-2xl font-semibold text-green-600">{batch.active}</span>
              <span className="text-sm text-gray-500">/ {batch.total} total</span>
            </div>
            <div className="mt-2 flex justify-between text-sm">
              <span className="text-gray-500">Used: {batch.used}</span>
              {batch.active > 0 && (
                <button
                  onClick={() => {
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {cards.map(card => (
              <tr key={card.id}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-mono">{card.code}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-mono">{card.pin}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm">${card.value?.toFixed(2)}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm">{card.days}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm">{card.service?.name || '-'}</td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-2 py-1 text-xs rounded-full ${card.is_used ? 'bg-gray-100 text-gray-600' : 'bg-green-100 text-green-800'}`}>
                    {card.is_used ? 'Used' : 'Available'}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{card.batch_id}</td>
        ...
        {meta.totalPages > 1 && (
          <div className="px-6 py-4 bg-gray-50 flex justify-between items-center">
            ...
            <div className="space-x-2">
              ...
              <button onClick={() => setPage(p => p + 1)} disabled={page >= meta.totalPages} className="px-3 py-1 bg-gray-200 rounded disabled:opacity-50">Next</button>
            </div>
          </div>
        )}
      </div>
      ...
      {showGenerateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-lg font-semibold mb-4">Generate Prepaid Cards</h2>
            <form onSubmit={handleGenerate} className="space-y-4">
              ...
              <div className="flex justify-end space-x-3 pt-4">
                <button type="button" onClick={() => setShowGenerateModal(false)} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200">Cancel</button>
      ...
      {showRedeemModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-lg font-semibold mb-4">Redeem Prepaid Card</h2>
            <form onSubmit={handleRedeem} className="space-y-4">
              ...
              <div className="flex justify-end space-x-3 pt-4">
                <button type="button" onClick={() => setShowRedeemModal(false)} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200">Cancel</button>
