...
func syncCDNPCQToNAS(cdnName string, speedLimitM int64, pcqLimit, pcqTotalLimit int, nasID uint, targetPools string) {
	companyName := getCDNCompanyName()
...
func syncCDNPCQToNASWithSubnets(cdnName string, speedLimitM int64, pcqLimit, pcqTotalLimit int, nasID uint, targetPools string, subnets []string) {
	companyName := getCDNCompanyName()
...
func cleanupPCQFromNAS(cdnName string, speedLimitM int64, nasID uint) {
	companyName := getCDNCompanyName()
	...
	if err := client.RemoveCDNPCQSetup(cdnName, speedLimitM, companyName); err != nil {
...
func syncCDNPCQToAllNAS(cdnName string, speedLimitM int64, pcqLimit, pcqTotalLimit int) {
	companyName := getCDNCompanyName()
...
func removeCDNPCQFromAllNAS(cdnName string) {
	...
	database.DB.Where("cdn_id = ?", cdn.ID).Find(&serviceCDNs)
	...
	toRemove := make(map[nasSpeed]bool)
	for _, sc := range serviceCDNs {
		if sc.PCQNASID != nil && sc.SpeedLimit > 0 {
			toRemove[nasSpeed{*sc.PCQNASID, sc.SpeedLimit}] = true
		}
	}

	for ns := range toRemove {
		go cleanupPCQFromNAS(cdnName, ns.speed, ns.nasID)
	}
}
	...
	for _, cdnConfig := range input.CDNs {
		...
		if cdnConfig.PCQEnabled && cdnConfig.IsActive && cdnConfig.SpeedLimit > 0 && cdnConfig.PCQNASID != nil && cdnConfig.PCQTargetPools != "" {
			...
			if err := database.DB.First(&cdn, cdnConfig.CDNID).Error; err == nil {
				...
				subnets := parseSubnets(cdn.Subnets)
				go syncCDNPCQToNASWithSubnets(cdn.Name, cdnConfig.SpeedLimit, pcqLimit, pcqTotalLimit, *cdnConfig.PCQNASID, cdnConfig.PCQTargetPools, subnets)
			}
		}
	}
...
func (h *CDNHandler) SyncPCQToNAS(c *fiber.Ctx) error {
	...
	go syncCDNPCQToAllNAS(cdn.Name, serviceCDN.SpeedLimit, serviceCDN.PCQLimit, serviceCDN.PCQTotalLimit)
...
func (h *CDNHandler) SyncAllPCQToNAS(c *fiber.Ctx) error {
	...
	var serviceCDNs []models.ServiceCDN
	database.DB.Preload("CDN").Where("pcq_enabled = ? AND is_active = ?", true, true).Find(&serviceCDNs)
	...
	for _, sc := range serviceCDNs {
		...
		syncCount++
		go syncCDNPCQToAllNAS(sc.CDN.Name, sc.SpeedLimit, sc.PCQLimit, sc.PCQTotalLimit)
	}
