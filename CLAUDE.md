...
## Recent Work
- **Resellers Can Use Customize Menu v1.0.259 (Feb 20, 2026)**: Changed `isAdmin()` to `(isAdmin() || isReseller())` on both mobile and desktop sidebar Customize Menu button. Resellers can now reorder and hide/show sidebar menu items. Frontend-only change. Files: `frontend/src/components/Layout.jsx`.
- **Reseller Balance in Header v1.0.258 (Feb 20, 2026)**: Added reseller balance display in top header bar next to clock. Shows `$X.XX` with `BanknotesIcon`, green for positive/red for negative. Only visible for resellers (`isReseller()` check). Auto-refreshes every 60 seconds via `refreshUser()`. Reads `user.reseller.balance` from auth store. Files: `frontend/src/components/Layout.jsx`.
- **Diagnostic Tools NAS Pre-select v1.0.257 (Feb 20, 2026)**: Fixed NAS dropdown not pre-selected when navigating to Diagnostic Tools. DiagnosticTools page now reads `?nas_id=X` URL param (`useSearchParams`) and immediately selects that NAS. NAS page now has a purple wrench icon button on each row that links to `/diagnostic-tools?nas_id={id}`. Auto-select also improved: applies URL param first, falls back to single-NAS auto-select. Files: `frontend/src/pages/DiagnosticTools.jsx`, `frontend/src/pages/Nas.jsx`.
- **Fix False 'Password Changed' in Audit Log v1.0.256 (Feb 20, 2026)**: Audit log showed "Password changed" on every subscriber save even when password wasn't changed. Root cause: frontend sends back the decrypted `password_plain` on every save; backend couldn't tell if it was different. Fix: compare submitted password against `security.DecryptPassword(subscriber.PasswordPlain)` before marking as changed. Files: `backend/internal/handlers/subscriber.go`.
- **Detailed Audit Log for Subscriber Updates v1.0.255 (Feb 20, 2026)**: Audit log now shows exact field changes: `Updated subscriber "info1633": Status: Active → Inactive, Price: $0.00 → $25.00`. Root cause of earlier failure: handler's direct `database.DB.Create(&auditLog)` silently failed because empty Go strings `""` are rejected by PostgreSQL `jsonb` columns. Fix: handler sets `c.Locals("audit_description/entity_id/entity_name")`, middleware reads them after `c.Next()` and uses them in the single audit entry. Added `buildSubscriberChanges()` helper that diffs old vs new subscriber state. Files: `backend/internal/handlers/subscriber.go`, `backend/internal/middleware/audit.go`.
- **Override Price for Subscribers v1.0.253 (Feb 20, 2026)**: Added per-subscriber price override on the edit page. Checkbox "Override service price for this subscriber" enables a custom price input; when unchecked shows the service's default price. `price` and `override_price` columns already existed in DB/model — only the Update handler `allowedFields` and frontend needed changes. Files: `backend/internal/handlers/subscriber.go`, `frontend/src/pages/SubscriberEdit.jsx`.
- **DSCP Direction for CDN Port Rules v1.0.252 (Feb 19, 2026)**: Added "DSCP Only" direction option to CDN Port Rules. Creates MikroTik mangle rule with `chain=postrouting`, `dscp=N`, no port/protocol matching. Port field hidden in UI when direction=dscp; DSCP value input (0-63) shown. Table shows orange "DSCP N" badge. Added `dscp_value INTEGER` column to `cdn_port_rules` table. **Critical lesson: always sync `cmd/api/main.go` to license server build source before building — new routes won't exist in binary otherwise.** Files: `backend/internal/models/cdn.go`, `backend/internal/models/schema.sql`, `backend/internal/mikrotik/client.go`, `backend/internal/handlers/cdn_port_rules.go`, `frontend/src/pages/CDNPortRules.jsx`.
- **CDN Torch Direction Fix v1.0.251 (Feb 19, 2026)**: Fixed CDN live graph direction matching. `txRate` always = download TO subscriber. Remote IP = whichever side is NOT the subscriber IP. Files: `backend/internal/mikrotik/client.go` (GetCDNTrafficViaTorch).
- **CDN Live Graph via MikroTik Torch v1.0.250 (Feb 19, 2026)**: Replaced connection-tracking CDN traffic detection with MikroTik `/tool/torch`. Previous approach returned 0 on servers with NAT or disabled connection tracking. Torch runs on subscriber's PPPoE interface, matches remote IPs against CDN subnets, returns bytes/sec rate directly. `cdn_is_rate=true` flag tells frontend to use rate without delta calculation. Files: `backend/internal/mikrotik/client.go` (GetCDNTrafficViaTorch), `backend/internal/handlers/subscriber.go`, `frontend/src/pages/Subscribers.jsx`.
- **CDN Port Rules Queue Priority v1.0.249 (Feb 18, 2026)**: Port rule simple queues now placed at position 0 (above user PPPoE queues) so they take priority. Files: `backend/internal/mikrotik/client.go` (SyncPortRule, move queue to top).
- **CDN Port Rules — Port-Based PCQ Speed Control v1.0.248 (Feb 18, 2026)**: New feature — port-based speed limiting on MikroTik. New "Port Rules" page in CDN section. Create rules with Name, Port, Direction (src/dst/both), Speed (Mbps), optional NAS target. Syncs to MikroTik: creates PCQ queue type + mangle rule(s) + simple queue at top. Files: `backend/internal/handlers/cdn_port_rules.go` (new), `backend/internal/models/cdn.go` (CDNPortRule model), `backend/internal/models/schema.sql`, `backend/internal/mikrotik/client.go` (SyncPortRule), `backend/cmd/api/main.go` (routes), `frontend/src/pages/CDNPortRules.jsx` (new).
- **Remove PCQ Auto-Repair Service v1.0.247 (Feb 18, 2026)**: Removed every-5-minute PCQ auto-sync background service (was hammering MikroTik API). Replaced with manual "Sync to MikroTik" button on CDN page. Files: `backend/internal/services/` (removed pcq auto-repair), `frontend/src/pages/CDN.jsx`.
- **Free Hours — Quota Discount v1.0.246 (Feb 18, 2026)**: Renamed "Time-Based Speed Control" to "Free Hours — Quota Discount". New meaning: ratio = % of usage that is FREE during time window. 100% = fully free (no quota counted). 70% = 30% counted. 0% = normal. Formula: `counted = delta × (100 - freePercent) / 100`. Removed erroneous ratio=0 bypass. Speed boost removed from this feature — use Bandwidth Rules separately. Files: `backend/internal/services/quota_sync.go`, `frontend/src/pages/Services.jsx`.
- **CDN/Services NAS Auto-Fill Fixes v1.0.239–245 (Feb 18, 2026)**: Series of fixes: (239) CDN config auto-fills NAS+pool from RADIUS settings. (240) Added `nas_id` to services table — NAS persists on edit/duplicate. Fixed React state ordering bug. (241) PCQ NAS hidden when service NAS known. (242) PCQ pool auto-selects matching pool. (243) Fixed auto-select bug (check for `/` not empty string). (244) PCQ pool list filtered to matching pool only. (245) Speed Limit field shows empty instead of "0". Files: `frontend/src/pages/Services.jsx`, `backend/internal/models/service.go`, `backend/internal/handlers/service.go`, `backend/internal/models/schema.sql`.
- **Install Script Fixes v1.0.238 (Feb 17, 2026)**: Fixed 5 bugs: (1) hardware_id formula mismatch causing read-only mode on fresh install — now matches API formula `stable|MAC|UUID|MID`. (2) Silent download failure — validates tar.gz before extraction. (3) `version=latest` handled correctly in download handler. (4) LUKS creation made sequential with `cryptsetup close` before creation. (5) Package directory corrected (`updates/` not `packages/`). Files: `install.sh`, `updates/install.sh`, license server `internal/handlers/build.go`.
- **Option 2 Secrets Fix v1.0.237 (Feb 17, 2026)**: Fixed JSON parsing for license secrets (flat format not nested). SERVER_IP now fetches public IP via api.ipify.org instead of using Docker internal IP. Files: `backend/internal/config/config.go`, `install.sh`.
- **Soft-Delete Username Fix v1.0.236 (Feb 17, 2026)**: Fixed 500 error when re-creating a subscriber with a previously deleted username. Root cause: GORM soft-delete only sets `deleted_at` but the `UNIQUE` constraint on `username` column still blocked re-inserting the same username. Fix: Changed to partial unique index `UNIQUE (username) WHERE deleted_at IS NULL` so soft-deleted rows don't block new inserts. Auto-applied on startup via EnsureIndexes. Files: `backend/internal/models/schema.sql`, `backend/internal/models/subscriber.go`, `backend/internal/database/database.go`.
- **Daily Usage Chart Fix v1.0.227 (Feb 14, 2026)**: Fixed daily usage chart in subscriber Usage tab not showing any days. Backend daily breakdown query (grouping radacct by day) was present but not returning data on old API binaries. Frontend x-axis was hardcoded to 31 days - now dynamically uses `dailyQuota.daily_download.length` or calculates actual days in current month. Data fallbacks changed from `Array(31).fill(0)` to `[]`. Files: `frontend/src/pages/SubscriberEdit.jsx` (chart x-axis fix), `backend/internal/handlers/subscriber.go` (daily breakdown query lines 527-584).
...
- **Enhanced Boot Security v1.0.222 (Feb 13, 2026)**: Three-layer password protection system to prevent Live USB attacks. (1) Local encrypted hash check - saves AES-256 encrypted root password hash to `/etc/proxpanel/shadow_hash.enc`, compared offline at every boot without network. (2) `chattr +i /etc/shadow` - makes shadow file immutable, blocking all password changes including via Live USB. (3) Network wait at boot - `After=network-online.target` ensures license server is contacted. Also added `verify-password` endpoint to license server, fixed missing `get_hardware_id` in LUKS keyscript, added `proxpanel-update-password` helper tool for legitimate password changes. Files: license server `internal/handlers/license.go`, `cmd/server/main.go`, `updates/install.sh`, customer scripts `verify-root-password`, `proxpanel-luks-keyscript`, `proxpanel-decrypt.service`.
- **Subscriber List UX v1.0.221 (Feb 13, 2026)**: Sticky action toolbar, red selected row highlights, custom delete modal with subscriber names, dark mode username color (cyan), clickable status counters (Online/Offline/Active/Inactive/Expired), reseller dropdown shows name, Created At field. Files: `frontend/src/pages/Subscribers.jsx`, `frontend/src/pages/SubscriberEdit.jsx`.
- **Reseller Deactivation Fix v1.0.220 (Feb 13, 2026)**: Fixed critical bug where deactivated resellers could still log in. Root cause: `resellers.is_active` was never checked during login or request processing - only `users.is_active` was checked. Fix: Added reseller active check in login handler (`auth.go`), auth middleware (`middleware/auth.go`), and synced `users.is_active` when toggling reseller status (`reseller.go`). Files: `internal/handlers/auth.go`, `internal/middleware/auth.go`, `internal/handlers/reseller.go`.
- **RADIUS Version Reporting Fix v1.0.219 (Feb 13, 2026)**: Fixed version showing "unknown" alternating with correct version. Root cause: RADIUS container didn't have `/opt` mount so couldn't read VERSION file, and both API+RADIUS containers overwrote the same DB row. Fix: (1) License server skips version update when incoming version is "unknown", (2) Added `./VERSION:/opt/proxpanel/VERSION:ro` volume mount to RADIUS container in docker-compose.yml. Files: `docker-compose.yml`, license server `internal/handlers/license.go`.
- **Network Interface Dropdown Fix v1.0.218 (Feb 13, 2026)**: Fixed Network Configuration dropdown not showing on single-interface VPS servers. Root cause: `availableInterfaces.length > 1` only showed dropdown when 2+ interfaces detected. Fix: Changed to `>= 1`. Files: `frontend/src/components/NetworkConfiguration.jsx` lines 415, 437.
- **Cross-Server Backup Restore v1.0.208-209 (Feb 12, 2026)**: Added V2 backup format with embedded license key for automatic cross-server restoration. Backups now include `LICENSE_KEY=` in header, system auto-detects and fetches correct DB password from license server. New endpoint: `GET /api/v1/license/backup-password`. Files: `internal/handlers/backup.go`, license server `internal/handlers/license.go`.
- **Network Configuration Multi-Interface v1.0.212-217 (Feb 12-13, 2026)**: Complete multi-interface support for Network Configuration page. Docker interface filtering (br-, veth, eth@), regex fix for @ symbols, IPv6-only interface handling, function scope fix, DNS auto-detection. Files: `frontend/src/components/NetworkConfiguration.jsx`, `backend/internal/handlers/network_config.go`.
...
### v1.0.228 Domain Switch + Grace Period (Feb 15, 2026)

**Two changes:**

#### 1. License Server Domain Switch: proxpanel.com → proxrad.com

All license server references switched from `license.proxpanel.com` to `license.proxrad.com`:
- `internal/license/client.go`: Grace period 7 days → 4 hours
- `install.sh`: LICENSE_SERVER URL
- `internal/handlers/backup.go`: Fallback URL
- `internal/security/certpin.go`: Certificate fingerprint updated for Cloudflare, hostname changed
- `internal/services/cluster_service.go`: Fallback URL
- `docker-compose.yml` + `docker-compose.prod.yml`: LICENSE_SERVER defaults

**License Server Nginx:** Both domains now accepted (`license.proxpanel.com license.proxrad.com`)

#### 2. Cloudflare Load Balancer

License server now fronted by Cloudflare with 2 origin servers:
- **Primary**: 109.110.185.33 (main license server)
- **Backup**: 188.93.113.5 (backup license server)
- DNS: `license.proxrad.com` → Cloudflare LB → origin servers
- SSL: Cloudflare manages certificates

**Nginx Config Change (on 109.110.185.33):**
```nginx
server_name license.proxpanel.com license.proxrad.com;
```

### v1.0.229 Install Script Overhaul + Fresh Install Fixes (Feb 15, 2026)

**Complete install script rewrite for fresh installs including ALL fixes from v1.0.219-228:**

#### Install Script Changes
- Added `iproute2` to API container packages (for Network Configuration)
- Added `/etc/netplan:/etc/netplan` volume mount (for Network Configuration)
- Added `privileged: true` and `pid: "host"` for API container
- Added `get_hardware_id()` function for consistent hardware binding
- Hardware ID sent during registration
- Secrets fetched from license server using hardware ID
- All references use `license.proxrad.com`

#### Docker-Compose Settings (Generated by Install Script)
| Setting | Value | Purpose |
|---------|-------|---------|
| `privileged: true` | API container | Access host network namespace |
| `pid: "host"` | API container | nsenter to host PID namespace |
| `/etc/netplan:/etc/netplan` | Volume mount | Read/write netplan configs |
| `iproute2` | Package | ip addr/route commands |
| `./VERSION:/opt/proxpanel/VERSION:ro` | RADIUS mount | Version reporting |
| `127.0.0.1:5432:5432` | DB port | Localhost-only (security) |
| `127.0.0.1:6379:6379` | Redis port | Localhost-only (security) |

#### Deployed To
- Main license server: 109.110.185.33 (`/opt/proxpanel-license/install.sh` + `/opt/proxpanel-license/updates/install.sh`)
- Backup license server: 188.93.113.5 (synced via SSH)
- Repo: `/root/ProRAD-temp/install.sh`

#### Verified On
- Fresh install server 139.162.152.240: ALL CHECKS PASSED
- Version: 1.0.229
- All 5 containers running and healthy
- Network Configuration API working (nsenter + interfaces + routes)
- LUKS encryption active
- Boot security services enabled

#### Comprehensive Audit Results (Feb 15, 2026)
- Source code: 89/89 Go files identical between repo and license server build source
